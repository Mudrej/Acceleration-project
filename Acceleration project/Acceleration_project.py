...
class Ui_MainWindow(object):
    ...
    def calculate(self):
        ...
        self.textEdit.clear()
        for i in self.data:
            self.textEdit.append(f'{i}: опрошено {self.data[i][1]} человек, средняя зарплата {self.data[i][0]} рублей, все данные: {str(self.data[i][2]).replace("[","").replace("]","")}')
    ...
    def save_dialog(self):
        ...
        with open(path,'w') as file:
            for i in self.data:
                file.write(f'{i}: опрошено {self.data[i][1]}, средняя зарплата {self.data[i][0]}, все данные: {str(self.data[i][2]).replace("[","").replace("]","")} \n\n')
